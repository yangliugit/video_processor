...
def video_synt(request_date: Item1):
    ...
    try:
        ...
        png_to_video_path = images_to_video(gif_to_png_path, 15)
        ...
        concat_video_path = concat_video_by_times(png_to_video_path, 15, times+1)
        globalLog.info("Generate videos success, path : %s " % concat_video_path)
        add_path = video_add_audio(concat_video_path, mp3_path, 15)
        globalLog.info("Video add Mp3 success, path : %s " % add_path)
        oss_path = upload_file_to_oss(add_path)
        if oss_path:
            remove_file(concat_video_path, add_path)
            ...
            globalLog.error("Video upload fail, url : %s " % add_path)
            return {"message": "fail", "time": time.time() - start, "video_url": oss_path}
